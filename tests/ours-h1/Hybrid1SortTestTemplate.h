#ifndef HYBRID1_SORT_TEST_TEMPLATE_H
#define HYBRID1_SORT_TEST_TEMPLATE_H
...
TYPED_TEST_P(HybridSortTestFixture, SortHybridTest) {
    ...
    Ciphertext<DCRTPoly> ctxt_out = directSort->sort_hybrid1(
    ...
    EXPECT_EQ(ctxt_out->GetLevel(), this->m_multDepth)
        << "Use the level returned by the result for better performance";

    Plaintext result;
    this->m_cc->Decrypt(this->m_privateKey, ctxt_out, &result);

    std::cout << "Final Level: " << ctxt_out->GetLevel() << std::endl;
