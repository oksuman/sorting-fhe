...
template <size_t N>
class KWay5SortTest : public ::testing::Test {
protected:
    void SetUp() override {
    ...
    template <size_t M>
    friend void RunKWay5SortTest(KWay5SortTest<M>* fixture);
};

template <size_t N>
void RunKWay5SortTest(KWay5SortTest<N>* fixture) {
    ...
    std::cout << "Using Ring Dimension: " << fixture->m_cc->GetRingDimension() << std::endl;
    std::cout << "Multiplicative depth: " << fixture->m_multDepth << std::endl;
    std::cout << "Scaling Mod: " << fixture->m_scaleMod << std::endl;

    auto ctxt = fixture->m_enc->encryptInput(inputArray);

    auto kwaySorter = std::make_unique<KWayAdapter<N>>(
        fixture->m_cc,
        fixture->m_publicKey,
        fixture->m_privateKey,
        fixture->m_enc,
        k, M);

    auto Cfg = SignConfig(CompositeSignConfig(3, d_f, d_g), fixture->m_multDepth);
    ...
    Plaintext result;
    fixture->m_cc->Decrypt(fixture->m_privateKey, ctxt_out, &result);
